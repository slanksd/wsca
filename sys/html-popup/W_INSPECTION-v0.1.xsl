...
<xsl:for-each select="FieldsDoc/Fields/Field">
	<xsl:choose>
		...
		<xsl:when test="FieldName='FIELD_REPORT'">
			<a target="_blank">
				<xsl:attribute name="href">Y:\EPM\zEPM_PublicShare\SLank\ArcGIS\Documents\<xsl:value-of select="FieldValue"/></xsl:attribute>
				Field Report
			</a>
		</xsl:when>
	</xsl:choose>
